...
This package is an extension of Apache Cordova Email Composer plugin (https://github.com/katzer/cordova-plugin-email-composer).
Several things were changed:
1) This version does not require android.permission.GET_ACCOUNTS permission (there is no check for active email account, it is assumed, that such account always exists).
2) This plugin opens directly default email application. It does not bother user to choose from all installed email clients (on Android).
